...
"""
<plugin key="Frisquet-connect" name="Frisquet-Connect" author="Krakinou refork by fj" version="0.3.1" wikilink="https://github.com/fjumeaux/FrisquetConnectDomoticz">
    <description>
...
class FrisquetConnectPlugin:
    ...
    def __init__(self):
        self.active = True
        self.httpConn = None
        self.httpConnData = None
        self.httpConnEnergy = None

        self.incomingPayload = None
        self.pendingPayload = None

        self.auth_token = None
        # token_expiry n'est plus utilisé (validité basée sur token_obtained_at + token_ttl_safe)
        self.token_expiry = 0
    ...
    def deviceUpdatedMoreThan(self, device, seconds):
        if device and device.Unit in Devices:
            ...
            try:
                last = dt(*_time.strptime(lu, "%Y-%m-%d %H:%M:%S")[0:6])
            except Exception:
    ...
    def writeEnergy(self, type_energy, date_str, energy, energy_total):
    ...
    def onMessage(self, Connection, Data):
        ...
        try:
            ...
            if not 200 <= Status < 300:
                ...
                if self.incomingPayload is not None and isinstance(self.incomingPayload, dict) and self.incomingPayload.get("message"):
                    Domoticz.Log(_("Server send an error %(status)d - %(message)s for %(name)s") % {
                        "status": Status,
                        "message": str(self.incomingPayload.get("message")),
            ...
            match Connection.Name:
                case "connectToFrisquetAPI":
                    if not self.incomingPayload or "token" not in self.incomingPayload:
                    ...
                    if self.retry_after_auth == "getFrisquetData":
                        self.retry_after_auth = None
                        if self.boilerID and self.is_token_valid():
                ...
                case "getFrisquetData":
                    ...
                    for zone in self.incomingPayload.get("zones", []):
                ...
                case _:
                    Domoticz.Error(_("Unknown connection: %s") % str(Connection.Name))
    ...
    def onHeartbeat(self):
        ...
        try:
            if not self.token_obtained_at:
                ...
                age_seconds = max(0, time.time() - float(self.token_obtained_at))
                age_hours = round(age_seconds / 3600, 2)
                age_str = str(age_hours).rstrip('0').rstrip('.').replace('.', ',')
        except Exception:
            age_str = "0"

        Domoticz.Status(_("Interrogation API Frisquet - âge du token : %s h") % age_str)
...
def DumpHTTPResponseToLog(httpResp, level=0):
    ...
    if isinstance(httpResp, dict):
        ...
        for x in httpResp:
            Domoticz.Debug(indentStr + "['" + str(x) + "']")
    else:
        Domoticz.Debug(indentStr + ">'" + str(httpResp) + "'")
